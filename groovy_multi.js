...
player.events.on("error",(queue,error)=>{
    ...
    queue=player.node.get(queue.guildId);
...
player.events.on("disconnect", (queue) => {
    ...
    connections.delete(queue.guildId);
});
...
player.events.on("emptyQueue", (queue) => {
    ...
    if(queue.connection.channel.member.size === 1){
...
client.on("messageCreate", async (message) => {
    ...
    if (message.content.includes("!deploy")) {
        await message.guild.commands.set([
            ...
            {
                name:"paulpager",
                description:"page paul with 5 messages at once"
            }
        ]);
...
client.on("interactionCreate", async (interaction) => {
    ...
    if (!interaction.member.voice.channel) {
        if (interaction.commandName === "ethan"){
            ...
            const image = await google.image(query,{safe:false});

            return interaction.followUp({content:`${query}\n${image[choice2].url}`});
        }else if(interaction.commandName==="20sideddie"){
            await interaction.deferReply();
    ...
    if (interaction.commandName === "play") {
        ...
        if(!queue){
            ...
            queue=player.node.get(interaction.guildId);
        }
