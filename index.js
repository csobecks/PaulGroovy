...
player.events.on('playerStart', (interaction, track) => {
    ...
    interaction.followUp({content:`Started playing: **${track.title}**`});
});

player.events.on('audioTrackAdd', (interaction, track) => {
    // Emitted when the player adds a single song to its queue
    const queue = useQueue(interaction.guild.id);
    interaction.followUp({content:`Track **${track.title}** queued`});
});

player.events.on('audioTracksAdd', (interaction) => {
    // Emitted when the player adds multiple songs to its queue
    const queue = useQueue(interaction.guild.id);
    interaction.followUp({content:`Multiple Track's queued`});
});

player.events.on('playerSkip', (interaction, track) => {
    // Emitted when the audio player fails to load the stream for a song
    const queue = useQueue(interaction.guild.id);
    interaction.followUp({content:`Skipping **${track.title}**`});
});

player.events.on('disconnect', (interaction) => {
    // Emitted when the bot leaves the voice channel
    const queue = useQueue(interaction.guild.id);
    interaction.followUp({content:'leaving now'});
});
...
player.events.on('emptyChannel', (interaction) => {
    ...
    const queue = useQueue(interaction.guild.id);
    interaction.followUp({content:`Leaving because no vc activity for the past 5 minutes`});
});

player.events.on('emptyQueue', (interaction) => {
    // Emitted when the player queue has finished
    const queue = useQueue(interaction.guild.id);
    interaction.followUp({content:'Queue finished!'});
});
